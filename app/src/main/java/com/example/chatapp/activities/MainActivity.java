...
public class MainActivity extends BaseActivity implements ConversionListener {
    ...
    private final EventListener<QuerySnapshot> eventListener = (value, error) -> {
        ...
        if (value != null) {
            for (DocumentChange dc : value.getDocumentChanges()) {
                if (dc.getType() == DocumentChange.Type.ADDED) {
                    ...
                    if (preferenceManager.getString(Constants.KEY_USER_ID).equals(senderId)) {
                        chatMessage.conversionImage = dc.getDocument().getString(Constants.KEY_RECEIVER_IMAGE);
                        ...
                        chatMessage.conversionImage = dc.getDocument().getString(Constants.KEY_SENDER_IMAGE);
                    ...
                    chatMessage.message = dc.getDocument().getString(Constants.KEY_LAST_MESSAGE);
                    ...
                    for (int i = 0; i < conversations.size(); i++) {
                        ...
                        if (conversations.get(i).senderId.equals(senderId) && conversations.get(i).receiverId.equals(receiverId)) {
                            conversations.get(i).message = dc.getDocument().getString(Constants.KEY_LAST_MESSAGE);
                            conversations.get(i).dateObject = dc.getDocument().getDate(Constants.KEY_TIMESTAMP);
                            break;
                        }
                    }
                }
            ...
            binding.conversionsRecycleView.smoothScrollToPosition(0);
            binding.conversionsRecycleView.setVisibility(View.VISIBLE);
